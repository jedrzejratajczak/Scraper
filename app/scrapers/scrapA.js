  ...
  const products = await Promise.all(
    items.map(async (item) => {
      const [src, price, title, href, productId] = await Promise.all([
        item.$eval('img.s-image', (img) => img.getAttribute('src')).catch(() => undefined),
        item.$eval('span.a-price span.a-offscreen', (el) => el.innerText).catch(() => undefined),
        item.$eval('h2.a-size-base-plus.a-color-base.a-text-normal span', (el) => el.innerText).catch(() => undefined),
        item
          .$eval('a.a-link-normal.s-underline-text.s-underline-link-text.s-link-style.a-text-normal', (el) =>
            el.getAttribute('href')
          )
          .catch(() => undefined),
        item.evaluate((el) => el.getAttribute('data-asin')).catch(() => undefined),
      ]);
