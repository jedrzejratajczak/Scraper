const acceptCookies = async (page) => {
  const acceptCookiesButton = await page.$('button#cookiejar-popup-main-button-accept');
...
const fetchProducts = async (page) => {
  const itemsContainerSelector = await page.waitForSelector('div.products-box-listing-results');
  ...
  for (const item of items) {
    ...
    const src = srcTag ? await page.evaluate((img) => img.src, srcTag) : undefined;
    const price = priceTag ? await page.evaluate((price) => price.textContent.trim(), priceTag) : undefined;
    const title = titleTag ? await page.evaluate((link) => link.textContent.trim(), titleTag) : undefined;
    const productId = cartButtonTag
      ? await page.evaluate((button) => button.getAttribute('data-product-id'), cartButtonTag)
      : undefined;
    const href = hrefTag ? await page.evaluate((link) => link.href, hrefTag) : undefined;
