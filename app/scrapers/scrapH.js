  ...
  const products = await Promise.all(
    items.map(async (item) => {
      const [src, title, href, productId] = await Promise.all([
        item.$eval('div.p-pict a img', (img) => img.getAttribute('src')).catch(() => undefined),
        item.$eval('div.product-title a.ng-binding', (el) => el.innerText).catch(() => undefined),
        item.$eval('div.product-title a.ng-binding', (el) => el.getAttribute('href')).catch(() => undefined),
        item.$eval('h3.ng-binding span.ref.ng-binding', (el) => el.innerText).catch(() => undefined),
      ]);
